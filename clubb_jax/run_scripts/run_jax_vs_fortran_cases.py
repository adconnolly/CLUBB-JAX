...
def _run_case(
    ...
    run_scm = repo_root / "clubb_jax" / "run_scripts" / "run_scm.py"
    run_bindiff = repo_root / "clubb_release" / "run_scripts" / "run_bindiff_all.py"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            "Stats file passed to run_scm.py. Relative paths are resolved "
            "against clubb_release first, then this repo."
        ),
    )
    parser.add_argument(
        "--max-iters",
...
def _resolve_stats_file(path_arg: str, repo_root: Path, clubb_release_root: Path) -> Path:
    path = Path(path_arg)
    if path.is_absolute():
        return path

    release_path = clubb_release_root / path
...
def main() -> int:
    ...
    clubb_release_root = repo_root / "clubb_release"
    ...
    stats_file = _resolve_stats_file(args.stats, repo_root, clubb_release_root)
    ...
    run_scm = repo_root / "clubb_jax" / "run_scripts" / "run_scm.py"
    run_bindiff = clubb_release_root / "run_scripts" / "run_bindiff_all.py"
