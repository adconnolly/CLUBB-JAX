...
def _clubb_release_root() -> Path:
    return _repo_root() / "clubb_release"
...
def _resolve_stats_registry_path(namelist_path: str, cfg: dict) -> Path:
    ...
    Priority:
      ...
      3) clubb_release default `input/stats/standard_stats.in`
    """
    ...
    return _clubb_release_root() / "input" / "stats" / "standard_stats.in"
...
def _resolve_case_input_path(namelist_dir: Path, runtype: str, suffix: str) -> Path:
    ...
    release_root = _clubb_release_root()
    fallback = release_root / "input" / "case_setups" / f"{runtype}{suffix}"
    if fallback.exists():
        return fallback
...
def _resolve_grid_file_path(namelist_dir: Path, grid_path_value) -> Path:
    ...
    if p.is_absolute():
        ...
        candidates.append(release_root / p)
        if raw.startswith("../input/"):
            candidates.append(repo_root / raw[3:])
            candidates.append(release_root / raw[3:])
