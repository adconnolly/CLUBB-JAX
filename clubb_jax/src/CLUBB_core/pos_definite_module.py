...
def pos_definite_adj(
    ...
    if gr.grid_dir_indx > 0:
        # Ascending grid
        kphalf = jnp.minimum(jnp.arange(nzt) + 1, nzm - 1)
        kmhalf = jnp.maximum(jnp.arange(nzt), 0)
    else:
        # Descending grid
        kphalf = jnp.maximum(jnp.arange(nzt), 0)
        kmhalf = jnp.minimum(jnp.arange(nzt) + 1, nzm - 1)

    # Eqn A4 from Smolarkowicz
    # We place a limiter of eps to prevent a divide by zero, and after this
    # calculation fout is on the scalar level, and fout is the total outward
    # flux for the scalar level k.
    fout = jnp.maximum(
