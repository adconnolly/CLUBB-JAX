...
def clip_covars_denom(
    ...
    # advance_wp2_wp3.
    upwp_denom = up2 if l_tke_aniso else wp2
    upwp, _ = clip_covar(nzm, ngrdcol, clip_upwp, wp2, upwp_denom, upwp)
    if l_linearize_pbl_winds:
        upwp_pert, _ = clip_covar(nzm, ngrdcol, clip_upwp, wp2, upwp_denom, upwp_pert)
    ...
    # advance_wp2_wp3.
    vpwp_denom = vp2 if l_tke_aniso else wp2
    vpwp, _ = clip_covar(nzm, ngrdcol, clip_vpwp, wp2, vpwp_denom, vpwp)
    if l_linearize_pbl_winds:
        vpwp_pert, _ = clip_covar(nzm, ngrdcol, clip_vpwp, wp2, vpwp_denom, vpwp_pert)
