...
def advance_xp3_simplified(
    ...
    # The Fortran code uses kp1 = max(k+1,nzt), so every interior k reads the
    # upper momentum level nzt after converting from 1-based indexing.
    kp1 = nzt - 1

    # Calculate the <x'^3> turbulent production (tp) term.
    term_tp_interior = term_tp_rhs(
        xp2_zt[:, :nzt - 1], wpxp[:, kp1:kp1 + 1], wpxp[:, :nzt - 1],
        rho_ds_zm[:, kp1:kp1 + 1], rho_ds_zm[:, :nzt - 1],
    ...
    term_ac_interior = term_ac_rhs(
        xm_zm[:, kp1:kp1 + 1], xm_zm[:, :nzt - 1], wpxp2[:, :nzt - 1],
        gr.invrs_dzt[:, :nzt - 1],
    )
