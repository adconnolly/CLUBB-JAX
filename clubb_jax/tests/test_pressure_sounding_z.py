...
def test_real_cgils_sounding():
    ...
    assert str(snd['alt_type']).strip().lower() == 'press[pa]', "cgils sounding is not pressure-coordinate"
    ...
    assert out['alt_type'] == 'z[m]', "alt_type not switched to z[m]"
...
def test_temperature_branch_independent():
    ...
    snd = dict(z=p, theta=T, rt=rt, alt_type='Press[Pa]', theta_type='T[K]', subs_type='omega[Pa/s]')
