#!/usr/bin/env python3
"""test_time_select.py — validate `time_select`, the time-bracket selector that mirrors every per-case
`<case>_read_t_dependent` Fortran reader (clex9_oct14/clex9_nov02/jun25/nov11) + the generic time-dependent forcing.
...
def test_interior_triples_match_fortran():
    ...
    for t in (10.0, 50.0, 99.999, 100.001, 175.0, 400.0, 549.0, 700.0, 899.0):
        jb, ja, jf = time_select(float(t), _TIMES)
...
def test_interpolation_equivalent_everywhere():
    ...
    for t in queries:
        jval = _interp(*time_select(float(t), _TIMES), _VALS)
    ...
    for i, t in enumerate(_TIMES):
        assert abs(_interp(*time_select(float(t), _TIMES), _VALS) - _VALS[i]) < 1e-12, f"node {i} not exact"
