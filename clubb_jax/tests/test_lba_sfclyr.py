...
def _ref(t, z, rho, thlm, ubar):
    ...
    ustar = _diag_ustar(z, bflx, ubar, _Z0)
...
def test_vs_literal():
    ...
    for _ in range(200):
        ...
        wth, wrt, ust = lba_sfclyr(t, z, rho, thlm, ubar)
...
def test_diurnal_structure():
    ...
    assert abs(float(lba_diurnal_factor(5.25 * 3600.0)) - 1.0) < 1e-12, "peak not at 5.25 h"
    # ft = 0 before t such that 0.5pi(5.25-t_hr)/5.25 >= pi/2 -> t_hr <= 0 ; and after the symmetric cutoff.
    assert float(lba_diurnal_factor(0.0)) >= 0.0 and float(lba_diurnal_factor(0.0)) < 1e-12, "ft(0) should be ~0"
    assert float(lba_diurnal_factor(12.0 * 3600.0)) == 0.0, "ft should be 0 well after the peak window"
    # Fluxes non-negative; ustar positive at the peak.
    wth, wrt, ust = lba_sfclyr(5.25 * 3600.0, 25.0, 1.1, 300.0, 4.0)
...
def test_differentiable():
    ...
    g = jax.grad(lambda r: lba_sfclyr(5.25 * 3600.0, 25.0, r, 300.0, 4.0)[0])(jnp.asarray(1.1))
    gt = jax.grad(lambda T: lba_sfclyr(5.25 * 3600.0, 25.0, 1.1, T, 4.0)[2])(jnp.asarray(300.0))
