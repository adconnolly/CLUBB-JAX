...
def test_soil_veg_vs_fortran_replica():
    ...
    got = advance_soil_veg(dt, rho, swup, swdn, lwdn, wpthlp, wprtp, psfc, deep, sfc, veg)
...
def test_soil_veg_init_and_integration():
    ...
    for _ in range(120):  # 1 hour
        deep, sfc, veg, shf = advance_soil_veg(dt, rho, swup, swdn, lwdn, wpthlp, wprtp, psfc, deep, sfc, veg)
...
def test_soil_veg_differentiable():
    ...
    def loss(lwdn, veg):
        _, _, veg_new, _ = advance_soil_veg(
            args['dt'], args['rho_sfc'], args['Frad_SW_up_sfc'], args['Frad_SW_down_sfc'], lwdn,
            args['wpthlp_sfc'], args['wprtp_sfc'], args['p_sfc'],
