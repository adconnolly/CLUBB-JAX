#!/usr/bin/env python3
"""test_smooth_heaviside.py — validate the JAX _smooth_heaviside_peskin_jax port (advance_helper_module.F90).
...
def test_f2py_is_truncated_constants():
    ...
    for smth_range in (0.6, 1.0, 0.25):
        ...
        worst_jax = max(worst_jax, np.max(np.abs(np.asarray(_smooth_heaviside_peskin_jax(x, smth_range)) - ref)))
...
def test_full_pi_closed_form_and_endpoints():
    ...
    got = np.asarray(_smooth_heaviside_peskin_jax(x, r))
    xr = x / r
    ...
    assert abs(float(np.asarray(_smooth_heaviside_peskin_jax(np.array([[0.0]]), r))[0, 0]) - 0.5) < 1e-14
...
def test_differentiable():
    r = 0.6
    g = np.asarray(jax.grad(lambda x: jnp.sum(_smooth_heaviside_peskin_jax(x, r) ** 2))(
        jnp.asarray(np.linspace(-0.5, 0.5, 11))))
    assert np.isfinite(g).all(), "non-finite grad through _smooth_heaviside_peskin_jax"
    print(f"  jax.grad through _smooth_heaviside_peskin_jax: finite ({g.size} entries)  PASS")
