#!/usr/bin/env python3
"""test_compute_gamma_skw.py — validate the JAX compute_gamma_Skw_jax port (Skx_module.F90:compute_gamma_Skw).
...
from clubb_jax.src.CLUBB_core.advance_xp3_module import compute_gamma_Skw_jax
...
def test_f2py_oracle():
    ...
    for l_gamma, params in configs:
        ...
        g_zm = np.asarray(compute_gamma_Skw_jax(skw_zm, params, l_gamma))
        g_zt = np.asarray(compute_gamma_Skw_jax(skw_zt, params, l_gamma))
...
def test_closed_form():
    ...
    got = np.asarray(compute_gamma_Skw_jax(skw, params, True))
...
def test_differentiable():
    ...
    def loss(skw):
        return jnp.sum(compute_gamma_Skw_jax(skw, params, True) ** 2)
    g = np.asarray(jax.grad(loss)(jnp.asarray(np.random.default_rng(1).uniform(-3, 3, (NG, NZM)))))
    assert np.isfinite(g).all(), "non-finite grad through compute_gamma_Skw_jax"
    print(f"  jax.grad through compute_gamma_Skw_jax: finite ({g.size} entries)  PASS")
