...
constants_clubb.py (e.g. `ibeta=61`, `imu=60`, `iSkw_denom_coef=73`) is a 1-based offset into the `clubb_params`
...
value. The bit-faithful cases imply it's correct, but nothing pinned it directly. This test pins it two ways:
  ...
  2. **index-constant self-consistency** — each `i<name>` constant indexes `PARAM_NAMES[i-1]` to exactly `<name>`
...
# Critical 1-based index constants → the PARAM_NAMES entry they must point at.
_INDEX_CONSTANTS = {
...
def test_index_constant_self_consistency():
    """Each i<name> index constant maps PARAM_NAMES[i-1] == <name> (no oracle needed)."""
    names = list(get_param_names())
    bad = []
    for const, expected in _INDEX_CONSTANTS.items():
        idx = getattr(cc, const, None)
        if idx is None:
            bad.append(f"{const}: not defined in constants_clubb"); continue
        got = names[idx - 1] if 1 <= idx <= len(names) else f"<out of range {idx}>"
        if got != expected:
            bad.append(f"{const}={idx} -> PARAM_NAMES[{idx-1}]={got!r}, expected {expected!r}")
...
def test_param_values_match_fortran():
    ...
    try:
        ...
        return
    fn = os.path.join(_ROOT, "clubb_release", "input", "tunable_parameters", "tunable_parameters.in")
