...
cross-check: it parses every `subroutine`/`function` name straight out of the Fortran `.F90` sources, then for
...
`derived_types/` type reorganization + the `advance_clubb_to_end` split). If anyone adds a routine to the wrong
file (its name matches a Fortran routine that lives in a different module), this fails with no dependence on the
audit's fold rules. Source-grounded; SKIPs cleanly if the Fortran tree is absent. (iter 528)
"""
import os
import re
import glob
...
#   * derived_types/pdf_params.py        ↔ pdf_parameter_module.F90  (the pdf_parameter type + its alloc/zero/
#     pack/unpack/print helpers live in the JAX-only derived_types/ package)
...
def _fortran_routine_homes():
    fort_dir = os.path.join(_ROOT, "clubb_release", "src")
    homes = {}
...
def test_routines_live_in_their_fortran_home_file():
    ...
    if not homes:
        print("  routine placement: SKIP (clubb_release Fortran sources absent)")
        return
    offenders = []
...
def test_documented_renames_are_all_live():
    ...
    homes = _fortran_routine_homes()
    if not homes:
        print("  documented-rename liveness: SKIP (Fortran sources absent)")
        return
    stale = []
