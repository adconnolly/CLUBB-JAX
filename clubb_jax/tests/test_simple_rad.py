...
from clubb_jax.src.Radiation.simple_rad_module import simple_rad, simple_rad_bomex, _CP, _LS_DIV
...
def test_simple_rad_clean():
    ...
    for f1 in (74.0, 0.0):           # F1>0 (both fluxes) and F1=0 (cloud-top only) branches
        for _ in range(8):
            ...
            f0, kappa = 70.0, 130.0
            frad_j, radht_j = simple_rad(gr, _NG, rho, rho_zm, rtm, rcm, exner,
                                         f0, f1, kappa, l_rad_above_cloud=False, l_sample=False)
...
def test_simple_rad_above_cloud():
    ...
    for _ in range(8):
        ...
        frad_j, radht_j = simple_rad(gr, _NG, rho, rho_zm, rtm, rcm, exner,
                                     f0, f1, kappa, l_rad_above_cloud=True, l_sample=False)
        ...
        frad = frad + rho_zm * _CP * _LS_DIV * heaviside * (0.25 * dz_pos ** (4.0 / 3.0) + z_i[:, None] * dz_pos ** (1.0 / 3.0))
        radht = (1.0 / exner) * (-1.0 / (_CP * rho)) * (frad[:, 1:] - frad[:, :-1]) * invrs_dzt
...
def test_simple_rad_bomex():
    ...
    j = np.asarray(simple_rad_bomex(zt))
    ref = np.where(zt < 1500.0, -2.315e-5,
                   np.where(zt < 2500.0, -2.315e-5 + 2.315e-5 * (zt - 1500.0) / 1000.0, 0.0))
    assert float(np.max(np.abs(j - ref))) == 0.0, "simple_rad_bomex != F90 piecewise profile"
