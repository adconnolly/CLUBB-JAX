...
Requires the clubb_python_api build on PYTHONPATH:
  export PYTHONPATH=$PWD:$PWD/clubb_release:$PWD/clubb_release/clubb_python_api
"""
import os
import sys
import tempfile
...
def call_f2py_advance_xm_wpxp(gr, nu, pdf_ic, err, a):
    ...
    result = clubb_f2py.f2py_advance_xm_wpxp(
        ...
        a["l_godunov_upwind_wpxp_ta"], a["l_upwind_xm_ma"], a["l_uv_nudge"], a["l_tke_aniso"],
...
def test_f2py_so_directly_callable():
    ...
    api_root = Path(__file__).resolve().parents[2] / "clubb_release" / "clubb_python_api"
