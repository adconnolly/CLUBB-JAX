...
from clubb_jax.src.CLUBB_core.calendar import (
    compute_current_date, gregorian2julian_date, julian2gregorian_date,
...
def test_compute_current_date_matches_reference():
    ...
    for d, m, y in _DATES:
        for secs in times:
            n += 1
            assert compute_current_date(d, m, y, secs) == _old_compute(d, m, y, secs), \
                f"compute_current_date mismatch at {(d, m, y, secs)}"
    print(f"  compute_current_date: matches month-walking reference over {n} cases  PASS")
...
def test_f2py_oracle():
    ...
    for d, m, y in _DATES:
        ...
        for secs in times:
            n += 1
            jx = compute_current_date(d, m, y, secs)
...
def main():
    print("test_calendar:")
    for t in (test_jdn_anchors_and_roundtrip, test_compute_current_date_matches_reference,
