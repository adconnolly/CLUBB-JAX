...
Complements test_no_dead_imports (dead imports) by catching dead DEFINITIONS — a top-level `def` that is:
  ...
  * and has ZERO call/reference sites anywhere in src + tests + run_scripts (only its own def line).
...
Pure-Python (reuses mirror_audit's Fortran-routine set); SKIPs cleanly if the Fortran oracle source is absent.
"""
import os
import re
import sys
...
sys.path.insert(0, os.path.join(_ROOT, "clubb_jax", "run_scripts"))
...
def _dead_functions():
    ...
    fort, _ = m._fortran_routines()
    corpus = ""
    for d in ("clubb_jax/src", "clubb_jax/tests", "clubb_jax/run_scripts"):
...
def _dead_private_helpers():
    ...
    references anywhere in src+tests+run_scripts beyond their own def line. `test_no_dead_functions` deliberately
...
def test_no_dead_functions():
    fort_src = os.path.join(_ROOT, "clubb_release", "src", "CLUBB_core")
    if not os.path.isdir(fort_src):
        print("  Fortran oracle source not present — SKIP")
        return
