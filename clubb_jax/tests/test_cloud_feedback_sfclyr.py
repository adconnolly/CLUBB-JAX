...
def test_vs_literal():
    ...
    psfc = rng.uniform(0.95e5, 1.02e5, n); Tsfc = rng.uniform(288, 305, n)
    wth, wrt, ust = cloud_feedback_sfclyr(*(jnp.asarray(a) for a in (thlm, rtm, zlow, ubar, psfc, Tsfc)), _SAT)
    wth = np.asarray(wth); wrt = np.asarray(wrt)
    worst = 0.0
    for i in range(n):
        rth, rrt = _ref(thlm[i], rtm[i], zlow[i], ubar[i], psfc[i], Tsfc[i])
        worst = max(worst, abs(wth[i] - rth), abs(wrt[i] - rrt))
    assert worst < 1e-13, f"vs literal {worst:.2e}"
    assert np.all(np.asarray(ust) == _USTAR), "ustar should be 0.3"
...
def test_physical_invariants():
    ...
    wth, wrt, _ = cloud_feedback_sfclyr(jnp.asarray([295.0]), jnp.asarray([0.012]), jnp.asarray([20.0]),
                                        jnp.asarray([5.0]), jnp.asarray([1.0e5]), jnp.asarray([298.0]), _SAT)
    ...
    wthw, wrtw, _ = cloud_feedback_sfclyr(jnp.asarray([290.0]), jnp.asarray([0.008]), jnp.asarray([30.0]),
                                          jnp.asarray([6.0]), jnp.asarray([1.0e5]), jnp.asarray([300.0]), _SAT)
...
def test_differentiable():
    g = jax.grad(lambda T: jnp.sum(cloud_feedback_sfclyr(
        jnp.asarray([295.0, 290.0]), jnp.asarray([0.01, 0.012]), jnp.asarray([25.0, 40.0]),
        jnp.asarray([5.0, 7.0]), jnp.asarray([1.0e5, 0.98e5]), T, _SAT)[0]))(jnp.asarray([298.0, 300.0]))
    assert np.isfinite(np.asarray(g)).all() and np.any(np.asarray(g) != 0.0), "grad bad"
    print(f"  jax.grad(cloud_feedback_sfclyr) wrt T_sfc: finite, nonzero (||g||={float(jnp.linalg.norm(g)):.3e})  PASS")
