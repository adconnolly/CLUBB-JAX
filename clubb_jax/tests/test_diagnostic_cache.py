...
the f2py Fortran `calculate_thvm`/`ddzt`/`zm2zt` primitives + the closed-form TKE (`1.5·wp2` isotropic /
`0.5·(wp2+up2+vp2)` anisotropic), with the `em_min` floor applied as `max(·, em_min)` over all levels (the Fortran
`zm2zt_api(…, em_min)` clips every level, grid_class.F90:2375). Both `l_tke_aniso` branches are covered.
    ...
    for l_tke_aniso in (False, True):
        ...
        for _ in range(12):
            ...
            thvm, em, sqrt_em_zt, ddzt_umvm_sqd = compute_diagnostic_cache(
                ...
                um=jnp.asarray(um), vm=jnp.asarray(vm), l_tke_aniso=l_tke_aniso, gr=jgr)

            # --- independent oracle composition ---
            thvm_ref = clubb_f2py.f2py_calculate_thvm(thlm, rtm, rcm, exner, thv_ds_zt)
            em_ref = (0.5 * (wp2 + up2 + vp2)) if l_tke_aniso else (1.5 * wp2)
        ...
        worst[f"aniso={l_tke_aniso}"] = dict(thvm=wt, em=wem, sqrt_em_zt=wsq, ddzt_umvm_sqd=wsh)

    overall = max(v for d in worst.values() for v in d.values())
    assert overall < 1e-12, f"compute_diagnostic_cache f2py mismatch: {worst}"
    print(f"  f2py compute_diagnostic_cache (thvm/em/sqrt_em_zt/ddzt_umvm_sqd, both l_tke_aniso branches): "
