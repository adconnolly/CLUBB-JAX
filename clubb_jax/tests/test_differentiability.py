...
cloud-fraction core, Brunt-Vaisala). NOTE: the full-timestep orchestration in
`advance_clubb_core_module.py` round-trips through NumPy at block boundaries
(np.asarray for in-place block updates), which severs the autodiff graph — so
end-to-end `jax.grad` through a whole CLUBB step is NOT yet available; that needs
the orchestration refactored to stay in jnp. The building blocks below ARE
differentiable today.
